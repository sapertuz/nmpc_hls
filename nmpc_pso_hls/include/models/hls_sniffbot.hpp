...
// #pragma HLS unroll
        phys_control[i] = value_map<_model_real>(control[i], u_min[i], u_max[i], phys_u_min[i], phys_u_max[i]);
    }
...
#pragma HLS pipeline off
    ...
    t2 = local_sin<_model_real>(phi   +  (_model_real)local_pi_half);
    t4 = local_sin<_model_real>(theta +  (_model_real)local_pi_half);
    t3 = local_sin<_model_real>(psi   +  (_model_real)local_pi_half);
    t5 = local_sin<_model_real>(phi);        
    t7 = local_sin<_model_real>(theta);
    t6 = local_sin<_model_real>(psi);
    ...
    t14 = t2*t2*t2;
    t16 = t4*t4;
    t17 = local_sin<_model_real>(t10);
    t18 = t5*t5;
    t19 = local_sin<_model_real>(t11);
    t20 = t7*t7;
