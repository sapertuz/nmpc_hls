...
public:
    ...
    void model(
        volatile _hw_real *state_dot, // [12]
        ...
        const _real     Iyy = 1.2;
        const _real     Izz = 2.3;
        const _real     l  = .25;     // Arm length [m]
        const _real     kf = 1;       // Thrust (lift) coefficient [Ns^2]
        const _real     kM = 0.2;     // Moment (drag) coefficient [Nms^2]
        const _real     m  = 2;       // Mass of quadcopter [kg]
        const _real     g  = 9.81;    // Gravity [m/s^2]
        const _real     b  = 4.9050;  // Speed offset [rad^2/sec^2]4.9050

        const _real M_mma[16] ={ 1,  1,  1,  1,  // thrust (T)
                        0, -1,  0,  1,  // roll  (tx)
                        -1,  0,  1,  0,  // pitch (ty)
                        -1,  1, -1,  1}; // yaw   (tz)
        const _real M_mma_inv[16] = { 0.2500,         0,   -0.5000,   -0.2500,
        ...
        // Input limits
        const _real phys_u_min[] = {-15,  -3,  -3,  -3};
        const _real phys_u_max[] = {15,   3,   3,   3};
...
// #pragma HLS bind_storage variable=u_max         type=ROM_1P impl=lutram
// #pragma HLS bind_storage variable=u_min         type=ROM_1P impl=lutram
// #pragma HLS bind_storage variable=M_mma         type=ROM_1P impl=lutram
// #pragma HLS bind_storage variable=M_mma_inv     type=ROM_1P impl=lutram
// #pragma HLS bind_storage variable=phys_u_min    type=ROM_1P impl=lutram
// #pragma HLS bind_storage variable=phys_u_max    type=ROM_1P impl=lutram
// #pragma HLS bind_storage variable=param         type=RAM_1P impl=lutram
// #pragma HLS bind_storage variable=u_param       type=RAM_1P impl=lutram
        //---------------------------------------------------
        _real t3;
        int j;
...
#pragma HLS pipeline off
        ...
        state_dot[11] = (t21*t22*(Izz*kM*u1*-(_real)2.0+Izz*kM*u2*(_real)2.0-Izz*kM*u3*(_real)2.0+Izz*kM*u4*(_real)2.0-
